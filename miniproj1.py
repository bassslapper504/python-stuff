...
db = sql.connect(host="localhost", user="root", passwd="12345", database="library")
cursor = db.cursor()
...
while (1):
    ...
    if ch == 4:
        try:
            ...
            print("ROLL NO", "STUDENT NAME", "CLASS", "SECTION", "SUBJECT1", "SUBJECT2", "SUBJECT3", "SUBJECT4",
                  "SUBJECT5", "TOTALMARKS", "PERCENTAGE")
            for i in data:
                j = str(i).split()
                for k in j:
                    print(k, end="     ")
    ...
    if ch == 5:
        ...
        t = m1 + m2 + m3 + m4 + m5
        per = t / 5
        query = "insert into student values(%d,'%s','%s','%s',%d,%d,%d,%d,%d,%d,%d)" % (
        r, name, c, s, m1, m2, m3, m4, m5, t, per)
    ...
    if ch == 7:
        try:
            ...
            if len(data) == 0:
                ...
                t = m1 + m2 + m3 + m4 + m5
                per = t / 5
                qry = "update STUDENT SET mark1=%d,mark2=%d,mark3=%d,mark4=%d,mark5=%d,total=%d,per=%d where roll=%d" % (
